...
public class PlayerMovement : MonoBehaviour {
    ...
    void Update() {
        ...
        var smoothedMovementFactor =
            isGrounded ? groundDamping : inAirDamping; // how fast do we change direction?
    ...
    public void recalculateDistanceBetweenRays() {
        ...
        var colliderUseableHeight = boxCollider.size.y * Mathf.Abs(localScale.y) - (2f * _skinWidth);
        _verticalDistanceBetweenRays = colliderUseableHeight / (totalHorizontalRays - 1);

        // vertical
        var colliderUseableWidth = boxCollider.size.x * Mathf.Abs(localScale.x) - (2f * _skinWidth);
    ...
    private void handleVerticalSlope(ref Vector3 deltaMovement) {
        ...
        if (_raycastHit) {
            ...
            if (Util.FloatEquals(angle, 0)) {
                return;
            }

            // we are moving down the slope if our normal and movement direction are in the same x direction
            var isMovingDownSlope = Util.FloatEquals(Mathf.Sign(_raycastHit.normal.x), Mathf.Sign(deltaMovement.x));
